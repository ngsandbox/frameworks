...
The example280__MarketPrice__Performance application is provided as an example
...
example280__MarketPrice__Performance is a simple performance focused application
...
Note: to effectively open up items in this example, please connect it up to 
	  custom build provider performance application, e.g. upajProvPerf, or 
	  rfajProvPerf.

Detailed Description
====================

example280__MarketPrice__Performance implements the following high level steps:

+ Implements OmmConsumerClient class in AppClient
  - overrides desired methods
  - provides own methods as needed, e.g. decode( FieldList )
    - the method decode( FieldList ) is a performance focused method
